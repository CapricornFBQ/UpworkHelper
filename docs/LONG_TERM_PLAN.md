...
1. SidePanel notes 保存失败路径 UI 测试仍需补专门断言。
...
这些测试通过真实 background message handler 调用业务入口。当前集中在 `scripts/validate_v0_2.mjs` 中 mock `chrome.storage.local`、`chrome.tabs.query`、`chrome.scripting.executeScript`、`fetch`，然后发送真实 message。
...
1. P1：补 SidePanel notes 保存失败路径 UI 测试。
2. P2：进入 v0.3 Profile review 前，补 `effectiveProfile`、conflict UI 和字段级 correction 测试。
