...
1. ClientRecord exact 自动匹配仍未实现，必须等待稳定 client URL/id 来源。
...
1. ClientRecord exact 自动匹配仍未进入当前实现，需要稳定 client URL/id 来源后再做。
