...
- Side Panel 展示 Opportunity、Snapshots、Notes、Extracted Fields、Score、Proposal Draft、Outcome，并支持 capture、profile extract、profile correction、score、proposal generate/edit/copy/archive、outcome record/void/filter、archive、notes 保存。
- 本地 `chrome.storage.local` 已拆出 `uosc_meta`、`uosc_settings`、`uosc_opportunities`、`uosc_snapshots`、`uosc_opportunity_profiles`、`uosc_score_results`、`uosc_note_revisions`、`uosc_my_profile`、`uosc_portfolio_cases`、`uosc_proposal_drafts`、`uosc_outcome_events`，并预留 Client、Selector、Analytics storage keys。
...
- 计划版本已绑定 Chrome extension 发布版本：`PLAN_VERSION = "0.6.0"`，`manifest.version = "0.6.0"`，并由 `scripts/validate_v0_6.mjs` 校验一致。
- 不做自动刷新、自动翻页、自动打开 Upwork 页面、自动提交 proposal。

当前已确认的基线差异和需要修正的问题：

- `draft` 已作为 legacy alias 保留；新 capture 写 `captured`，评分成功写 `scored`。`scripts/validate_v0_6.mjs` 已用 legacy fixture 验证迁移和幂等。
- Capture 严格 Upwork host 和 jobKey 校验已落地。`scripts/validate_v0_6.mjs` 已覆盖 `capture:currentPage` 成功/失败路径的真实 handler 测试。
...
当前状态：`[v0.6 最小闭环已实现，已验证]`。Side Panel 已提供 Outcome filter、Outcome 表单、Record outcome、Void selected event 和 timeline；background 已提供 `outcome:appendEvent`、`outcome:listEvents`、`outcome:getSummary`、`outcome:voidEvent`，并兼容 `outcome:create/list/delete` alias；`uosc_outcome_events` 已纳入 read/write/import/export/permanent delete 级联；`OutcomeSummary` 只从事件流派生，不作为事实写回。`capture:currentPage` 在用户手动采集页面时会按保守短语追加 `capture_detected_status` 事件。验证见 `scripts/validate_v0_6.mjs` 的 append/list/summary/void、proposal_sent payload、capture detected、import 引用校验和 permanent delete 级联 case，以及 `scripts/smoke_unpacked_extension.mjs` 的真实扩展 Outcome UI smoke。
...
1. Capture 范围和权限模型。
   ...
   - 验证：`scripts/validate_v0_6.mjs` 通过真实 handler 覆盖非 Upwork、无文本、新建、追加、jobKey mismatch、archived 拒绝、list/detail 分离。
...
4. `[已确认，已实施]` 当前核心回归测试先用 plain Node 脚本和静态扫描，不引入 Jest / Vitest；`scripts/validate_v0_6.mjs` 已升级为真实 background handler harness。
...
8. `ProposalDraft.status` 已在 v0.5 进入运行时代码；`OutcomeEvent.eventType` 仍保留在文档 contract，运行时代码进入 v0.6。
...
6. Risk static check、manifest JSON parse、JS syntax check 已进入 `scripts/validate_v0_6.mjs`。
...
1. ClientRecord、Selector、Analytics 尚未实现。
...
3. background 已拆出 `uosc_meta`、`uosc_snapshots`、`uosc_opportunity_profiles`、`uosc_score_results`、`uosc_note_revisions`、`uosc_proposal_drafts`、`uosc_outcome_events`，并保留旧数据迁移兼容。
4. Options 已提供 storage usage、backup、export、import preview、import commit；导出结果不包含 API Key。
5. `scripts/validate_v0_6.mjs` 已覆盖 JS syntax、风险静态扫描、adapter fake response、migration、settings、capture、archive/restore/delete、notes stale、profile review、My Profile / Portfolio、Proposal Draft、OutcomeEvent、score、backup/export/import 的真实 handler 路径。
6. MCP / Playwright 已用 mocked `chrome.runtime` 验证 Options 数据 UI 的导出、预览导入、确认导入流程。
7. `PLAN_VERSION = "0.6.0"` 已和 Chrome extension `manifest.version = "0.6.0"` 绑定，并由校验脚本断言一致。

仍未进入当前实现或仍未验证的内容：

1. ClientRecord、Selector、Analytics 仍未进入当前实现。
...
| OutcomeEvent | `outcome:appendEvent/listEvents/getSummary/voidEvent` | 追加事件；读取事件流和派生 summary；void event 不物理删除；Opportunity 不保存最终 outcome 字段 | 真实 handler 已覆盖 append/list/summary/void、capture detected、import payload 引用校验；SidePanel smoke 覆盖 Outcome UI | v0.6 最小闭环已验证；Client/Analytics 尚未实现 |
...
| 风险边界 | `scripts/validate_v0_6.mjs` 静态扫描 | 检查自动化高风险 API 和 Upwork 私有 API 文本 | 有静态测试 | 只能证明代码文本没有命中，不能替代扩展运行时 smoke |

#### 9.9.2 已确认测试是否调用真实业务逻辑

当前 `scripts/validate_v0_6.mjs` 已真实调用：
...
MCP / Playwright 当前没有覆盖真实 background 业务逻辑，因为测试注入的是 mocked `chrome.runtime.sendMessage`。因此它只能证明 Options UI 按钮和状态文案流程正确；真实业务逻辑由 `scripts/validate_v0_6.mjs` 的 handler harness 覆盖。

#### 9.9.3 已纳入真实业务逻辑测试的 core case

这些测试通过真实 background message handler 调用业务入口。当前集中在 `scripts/validate_v0_6.mjs` 中 mock `chrome.storage.local`、`chrome.tabs.query`、`chrome.scripting.executeScript`、`fetch`，然后发送真实 message。
...
1. 测试覆盖不足。
   - 状态：`[已大幅修复]`。
   - 证据：`scripts/validate_v0_6.mjs` 已建立 background harness，以真实 message handler 覆盖 9.9.3 的 core case。
...
11. 未实现业务域不应被误判为完成。
    - 问题：ClientRecord、Analytics、Selector 目前只有 schema key 或文档计划，没有完整业务闭环。
...
13. 未来实体 import validation 不完整。
    - 状态：`[已修复当前边界]`。
    - 决策：已实现的 My Profile / Portfolio / ProposalDraft / OutcomeEvent 允许导入并校验 shape/ref/sourceRefs/payload；仍未实现的未来实体暂时拒绝非空导入；到对应版本实现时再补完整 validator 和引用校验。
    - 证据：`validateUnimplementedImportKeysAreEmpty()` 拒绝非空 `uosc_client_records` 等未来实体；ProposalDraft import 已校验 `inputScoreResultId` 和 `sourceRefs` 坏引用；OutcomeEvent import 已校验 proposal draft 和 detectedStatus 坏引用。

14. 计划版本和扩展发布版本关系不明确。
    - 状态：`[已修复]`。
    - 决策：计划版本绑定 Chrome extension `manifest.version`。
    - 证据：`src/shared/schema.js` 导出 `PLAN_VERSION = "0.6.0"`，`manifest.json` 使用 `version = "0.6.0"`，校验脚本断言两者一致。
...
1. P4：进入 v0.7 ClientRecord 前，补 ClientRecord identity/merge/split contract、手动匹配 UI、引用校验和真实 handler 测试。
