...
- `draft` 已作为 legacy alias 保留；新 capture 写 `captured`，评分成功写 `scored`。`scripts/validate_v0_2.mjs` 已用 legacy fixture 验证迁移和幂等。
- Capture 严格 Upwork host 和 jobKey 校验已落地。`scripts/validate_v0_2.mjs` 已覆盖 `capture:currentPage` 成功/失败路径的真实 handler 测试。
...
1. Capture 范围和权限模型。
   ...
   - 验证：`scripts/validate_v0_2.mjs` 通过真实 handler 覆盖非 Upwork、无文本、新建、追加、jobKey mismatch、archived 拒绝、list/detail 分离。
...
9. Storage 长期容量和写入并发未设计。
   ...
   - 剩余修复：snapshot redacted/compacted UI 未实现。
...
10. Import / Export 的隐私边界不完整。
    ...
    - 剩余修复：完整 Chrome extension runtime smoke 仍未覆盖。
...
4. `[已确认，已实施]` v0.2 测试先用 plain Node 脚本和静态扫描，不引入 Jest / Vitest；`scripts/validate_v0_2.mjs` 已升级为真实 background handler harness。
...
10. notes 真实 handler 覆盖 revision 历史和 `scoreStale`；Side Panel 保存 notes 已有错误处理。

待验证/待实现：

1. Snapshot retention 清理 UI 未实现。
2. Chrome unpacked extension runtime smoke 未完成。
...
2. Capture：
   ...
   - 缺口：没有 snapshot 单独 CRUD；没有 redacted/compacted 清理 UI。
...
3. Opportunity：
   ...
   - 缺口：permanent delete 还没有 UI 暴露和风险确认文案。
...
5. `scripts/validate_v0_2.mjs` 已覆盖 JS syntax、风险静态扫描、adapter fake response、migration、settings、capture、archive/restore/delete、notes stale、score、backup/export/import 的真实 handler 路径。
...
| Opportunity + Snapshot Capture | `capture:currentPage`、`opportunities:listSummary`、`opportunities:get` | capture 创建 Opportunity/Snapshot；list/detail 读取；追加 snapshot 更新 Opportunity | 真实 handler 已覆盖成功和失败路径 | 核心闭环已验证；snapshot retention UI 未实现 |
| Opportunity archive/restore/delete | `opportunities:archive`、`opportunities:restore`、`opportunities:deletePermanent` | archive 软删除；restore 恢复；permanent delete 级联删除关联数据 | 真实 handler 已覆盖列表过滤、restore、级联删除 | 逻辑已验证；permanent delete 仍未在 UI 暴露 |
...
| Profile + Score | `score:opportunity` | 调 OpenAI 提取 profile，再评分，创建 `OpportunityProfile` 和 `ScoreResult`，更新 current ids | 真实 handler 已覆盖 fake OpenAI 成功、clamp、invalid JSON、HTTP error、并发 notes 冲突 | 评分闭环已验证；profile review/history UI 未实现 |
...
MCP / Playwright 当前没有覆盖真实 background 业务逻辑，因为测试注入的是 mocked `chrome.runtime.sendMessage`。因此它只能证明 Options UI 按钮和状态文案流程正确；真实业务逻辑由 `scripts/validate_v0_2.mjs` 的 handler harness 覆盖。
...
1. 测试覆盖不足。
   - 状态：`[已大幅修复]`。
   - 证据：`scripts/validate_v0_2.mjs` 已建立 background harness，以真实 message handler 覆盖 9.9.3 的 core case。
   - 剩余：Chrome unpacked extension runtime smoke 仍未覆盖真实扩展上下文。
...
9. SidePanel 保存 notes 缺少错误处理。
   - 状态：`[已修复代码，待 UI 失败路径测试]`。
   - 证据：`saveNotes()` 已增加 busy/error handling。
...
1. P1：补 Chrome unpacked extension smoke，确认真实扩展上下文可加载。
2. P1：补 SidePanel notes 保存失败路径 UI 测试。
3. P2：补 Snapshot retention UI 和测试。
4. P2：进入 v0.3 Profile review 前，补 `effectiveProfile`、conflict UI 和字段级 correction 测试。
