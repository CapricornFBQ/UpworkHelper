...
- Side Panel 展示 Opportunity、Snapshots、Notes、Extracted Fields、Score、Proposal Draft，并支持 capture、profile extract、profile correction、score、proposal generate/edit/copy/archive、archive、notes 保存。
- 本地 `chrome.storage.local` 已拆出 `uosc_meta`、`uosc_settings`、`uosc_opportunities`、`uosc_snapshots`、`uosc_opportunity_profiles`、`uosc_score_results`、`uosc_note_revisions`、`uosc_my_profile`、`uosc_portfolio_cases`、`uosc_proposal_drafts`，并预留 Outcome、Client、Selector、Analytics storage keys。
...
- 计划版本已绑定 Chrome extension 发布版本：`PLAN_VERSION = "0.5.0"`，`manifest.version = "0.5.0"`，并由 `scripts/validate_v0_5.mjs` 校验一致。
- 不做自动刷新、自动翻页、自动打开 Upwork 页面、自动提交 proposal。

当前已确认的基线差异和需要修正的问题：

- `draft` 已作为 legacy alias 保留；新 capture 写 `captured`，评分成功写 `scored`。`scripts/validate_v0_5.mjs` 已用 legacy fixture 验证迁移和幂等。
- Capture 严格 Upwork host 和 jobKey 校验已落地。`scripts/validate_v0_5.mjs` 已覆盖 `capture:currentPage` 成功/失败路径的真实 handler 测试。
...
1. Capture 范围和权限模型。
   ...
   - 验证：`scripts/validate_v0_5.mjs` 通过真实 handler 覆盖非 Upwork、无文本、新建、追加、jobKey mismatch、archived 拒绝、list/detail 分离。
...
4. `[已确认，已实施]` 当前核心回归测试先用 plain Node 脚本和静态扫描，不引入 Jest / Vitest；`scripts/validate_v0_5.mjs` 已升级为真实 background handler harness。
...
6. Risk static check、manifest JSON parse、JS syntax check 已进入 `scripts/validate_v0_5.mjs`。
...
1. v0.6 Outcome Tracking 尚未实现。
2. ClientRecord、Selector、Analytics 尚未实现。
...
5. `scripts/validate_v0_5.mjs` 已覆盖 JS syntax、风险静态扫描、adapter fake response、migration、settings、capture、archive/restore/delete、notes stale、profile review、My Profile / Portfolio、Proposal Draft、score、backup/export/import 的真实 handler 路径。
6. MCP / Playwright 已用 mocked `chrome.runtime` 验证 Options 数据 UI 的导出、预览导入、确认导入流程。
7. `PLAN_VERSION = "0.5.0"` 已和 Chrome extension `manifest.version = "0.5.0"` 绑定，并由校验脚本断言一致。

仍未进入当前实现或仍未验证的内容：

1. OutcomeEvent、ClientRecord、Selector、Analytics 仍未进入当前实现。
...
| 风险边界 | `scripts/validate_v0_5.mjs` 静态扫描 | 检查自动化高风险 API 和 Upwork 私有 API 文本 | 有静态测试 | 只能证明代码文本没有命中，不能替代扩展运行时 smoke |

#### 9.9.2 已确认测试是否调用真实业务逻辑

当前 `scripts/validate_v0_5.mjs` 已真实调用：

1. `src/shared/adapters.js` 的 `mapRawProfileFields()`。
2. `src/shared/adapters.js` 的 `normalizeRawScore()`。
3. background 注册到 `chrome.runtime.onMessage` 的真实 handler，目前覆盖 migration、settings、capture、archive/restore/delete、notes、score、proposal、backup/export/import。
...
MCP / Playwright 当前没有覆盖真实 background 业务逻辑，因为测试注入的是 mocked `chrome.runtime.sendMessage`。因此它只能证明 Options UI 按钮和状态文案流程正确；真实业务逻辑由 `scripts/validate_v0_5.mjs` 的 handler harness 覆盖。

#### 9.9.3 已纳入真实业务逻辑测试的 core case

这些测试通过真实 background message handler 调用业务入口。当前集中在 `scripts/validate_v0_5.mjs` 中 mock `chrome.storage.local`、`chrome.tabs.query`、`chrome.scripting.executeScript`、`fetch`，然后发送真实 message。
...
6. Archive / Restore / Permanent Delete：
   ...
   - permanent delete 会删除 Opportunity、Snapshots、Profiles、Scores、Notes、ProposalDrafts。
...
1. 测试覆盖不足。
   - 状态：`[已大幅修复]`。
   - 证据：`scripts/validate_v0_5.mjs` 已建立 background harness，以真实 message handler 覆盖 9.9.3 的 core case。
...
11. 未实现业务域不应被误判为完成。
    - 问题：OutcomeEvent、ClientRecord、Analytics、Selector 目前只有 schema key 或文档计划，没有完整业务闭环。
...
13. 未来实体 import validation 不完整。
    - 状态：`[已修复当前边界]`。
    - 决策：已实现的 My Profile / Portfolio / ProposalDraft 允许导入并校验 shape/ref/sourceRefs；仍未实现的未来实体暂时拒绝非空导入；到对应版本实现时再补完整 validator 和引用校验。
    - 证据：`validateUnimplementedImportKeysAreEmpty()` 拒绝非空 `uosc_outcome_events` 等未来实体；ProposalDraft import 已校验 `inputScoreResultId` 和 `sourceRefs` 坏引用。

14. 计划版本和扩展发布版本关系不明确。
    - 状态：`[已修复]`。
    - 决策：计划版本绑定 Chrome extension `manifest.version`。
    - 证据：`src/shared/schema.js` 导出 `PLAN_VERSION = "0.5.0"`，`manifest.json` 使用 `version = "0.5.0"`，校验脚本断言两者一致。
...
1. P4：进入 v0.6 Outcome Tracking 前，补 OutcomeEvent CRUD contract、手动投标/结果记录 UI、事件引用校验和真实 handler 测试。
...
v0.2 runtime 必须先把当前已实现业务所需常量放进 `src/shared/schema.js`，实现和测试都引用同一份定义。`ProposalDraftStatus` 已在 v0.5 进入 runtime schema；`OutcomeEvent.eventType` 等 future enum 可以先保留在本文档，到 v0.6 实现时再进入 runtime schema。
