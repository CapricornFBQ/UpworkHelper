...
- 本地 `chrome.storage.local` 已拆出 `uosc_meta`、`uosc_settings`、`uosc_opportunities`、`uosc_snapshots`、`uosc_opportunity_profiles`、`uosc_score_results`、`uosc_note_revisions`、`uosc_my_profile`、`uosc_portfolio_cases`、`uosc_proposal_drafts`、`uosc_outcome_events`、`uosc_client_records`、`uosc_field_selectors`，并预留 Analytics cache key。
...
- 计划版本已绑定 Chrome extension 发布版本：`PLAN_VERSION = "0.9.0"`，`manifest.version = "0.9.0"`，并由 `scripts/validate_v0_9.mjs` 校验一致。
- 不做自动刷新、自动翻页、自动打开 Upwork 页面、自动提交 proposal。

当前已确认的基线差异和需要修正的问题：

- `draft` 已作为 legacy alias 保留；新 capture 写 `captured`，评分成功写 `scored`。`scripts/validate_v0_9.mjs` 已用 legacy fixture 验证迁移和幂等。
- Capture 严格 Upwork host 和 jobKey 校验已落地。`scripts/validate_v0_9.mjs` 已覆盖 `capture:currentPage` 成功/失败路径的真实 handler 测试。
...
- 当前 `selectors:*` 已提供 FieldSelector 最小闭环；Selector Assist 只由用户在 Side Panel 点击启动，capture 会读取同 host/pageType 的 active selector，成功写入 selector match，失败写入 lastFailure 并在 UI 中显示。
...
当前状态：`[v0.6 最小闭环已实现，已验证]`。Side Panel 已提供 Outcome filter、Outcome 表单、Record outcome、Void selected event 和 timeline；background 已提供 `outcome:appendEvent`、`outcome:listEvents`、`outcome:getSummary`、`outcome:voidEvent`，并兼容 `outcome:create/list/delete` alias；`uosc_outcome_events` 已纳入 read/write/import/export/permanent delete 级联；`OutcomeSummary` 只从事件流派生，不作为事实写回。`capture:currentPage` 在用户手动采集页面时会按保守短语追加 `capture_detected_status` 事件。验证见 `scripts/validate_v0_9.mjs` 的 append/list/summary/void、proposal_sent payload、capture detected、import 引用校验和 permanent delete 级联 case，以及 `scripts/smoke_unpacked_extension.mjs` 的真实扩展 Outcome UI smoke。
...
当前状态：`[v0.7 最小闭环已实现，已验证]`。Side Panel 已提供 Client history、ClientRecord 创建/更新、当前 Opportunity 关联/解除、归档、当前机会拆分、客户合并入口和 `Pick employer link` identity picking；background 已提供 `clients:list`、`clients:get`、`clients:create`、`clients:startIdentityPicking`、`clients:update`、`clients:archive`、`clients:linkOpportunity`、`clients:unlinkOpportunity`、`clients:merge`、`clients:split`；`uosc_client_records` 已纳入 read/write/import/export，并校验 `Opportunity.clientRecordId`、`identitySources`、active `primaryClientKey` 唯一性。`seenCount`、`averageScore`、`previousOutcomes` 只从事实表派生。当前实现不根据页面文本自动推断 probable client；exact 自动关联仍待下一步基于已提取的 exact key 实现。验证见 `scripts/validate_v0_9.mjs` 的 create/list/get/update/link/unlink/archive/merge/split、startIdentityPicking、import 引用校验、permanent delete 清理 identity source，以及 `scripts/smoke_unpacked_extension.mjs` 的真实扩展 Client history UI smoke。
...
当前状态：`[v0.9 最小闭环已实现，已验证]`。Side Panel 已提供 Selector Assist 字段选择、pageType 选择、Pick selector、Extract selectors、active selector 列表和 archive 入口；background 已提供 `selectors:list/create/update/archive/startPicking/extractForCurrentPage`。点选脚本只在用户点击后注入到当前 active Upwork tab，监听 click / Esc，结束后清理 overlay 和事件监听；capture 只读取保存的 selector，不后台轮询、不自动打开 Upwork 页面、不自动提交。selector 成功会写入 snapshot `stats.selectorMatches` 并可覆盖 profile field source 为 `selector`；selector 失败会写入 snapshot `stats.selectorFailures` 和 FieldSelector `lastFailure`，不会被 DOM visible text / AI fallback 静默掩盖。验证见 `scripts/validate_v0_9.mjs` 的 create/list/update/archive/startPicking/capture success/failure/profile selector source/import 校验 case，以及 `scripts/smoke_unpacked_extension.mjs` 的真实扩展 SidePanel Selector Assist UI smoke。
...
- 当前 v0.9 最小实现未新增文件，沿用现有 `src/background/background.js` 和 `src/sidepanel/sidepanel.js`，其中 picking/extract 函数通过 `chrome.scripting.executeScript` 临时注入。
...
- `src/shared/schema.js`
- `scripts/validate_v0_9.mjs`
- `scripts/smoke_unpacked_extension.mjs`
...
- 当前 v0.9 最小实现先沿用单文件 background / sidepanel 结构，未拆 `src/background/selectors.js` 或 `src/sidepanel/selector-panel.js`。
...
1. Capture 范围和权限模型。
   ...
   - 验证：`scripts/validate_v0_9.mjs` 通过真实 handler 覆盖非 Upwork、无文本、新建、追加、jobKey mismatch、archived 拒绝、list/detail 分离。
...
3. `ProfileField` 多来源结构。
   ...
   - 证据：v0.9 Selector Assist 已把 selector match 合并为 `effectiveSource: "selector"`，并保留 selector source。
...
7. ClientRecord 的 exact match 条件不够严格。
   - 状态：`[人工闭环已实现；exact identity picking 已实现；exact 自动匹配未实现]`。
   ...
   - 当前 UI：Client history 区域已提供 `Pick employer link`，用户点击后在当前 Upwork 页点选雇主主页链接，插件提取 link `href` 并写入 `exact:upwork-client-url:<url>` Identity key；如果页面拿不到稳定链接，再手动输入。
...
4. `[已确认，已实施]` 当前核心回归测试先用 plain Node 脚本和静态扫描，不引入 Jest / Vitest；`scripts/validate_v0_9.mjs` 已升级为真实 background handler harness。
...
6. Risk static check、manifest JSON parse、JS syntax check 已进入 `scripts/validate_v0_9.mjs`。
...
18. Selector Assist 真实 handler 覆盖 selector create/list/update/archive、用户触发 startPicking、capture success/failure、extract failure、profile selector source 和 import validator。

待验证/待实现：

1. ClientRecord exact 自动匹配仍未实现；当前已可提取稳定 client URL/id 到 Identity key，下一步可用该 exact key 自动 link 已有 ClientRecord。
...
5. `scripts/validate_v0_9.mjs` 已覆盖 JS syntax、风险静态扫描、adapter fake response、migration、settings、capture、archive/restore/delete、notes stale、profile review、My Profile / Portfolio、Proposal Draft、OutcomeEvent、ClientRecord、Selector Assist、Analytics、score、backup/export/import 的真实 handler 路径。
6. MCP / Playwright 已用 mocked `chrome.runtime` 验证 Options 数据 UI 的导出、预览导入、确认导入流程。
7. `PLAN_VERSION = "0.9.0"` 已和 Chrome extension `manifest.version = "0.9.0"` 绑定，并由校验脚本断言一致。

仍未进入当前实现或仍未验证的内容：

1. ClientRecord exact 自动匹配仍未进入当前实现；当前已具备 exact identity picking，下一步可用 exact key 自动 link 已有 ClientRecord。
...
| ClientRecord | `clients:create/list/get/update/linkOpportunity/unlinkOpportunity/archive/merge/split/startIdentityPicking` | 创建客户；读取列表/详情；提取 exact identity key；更新 identity/notes/redFlags；Opportunity 手动关联/解除；客户合并/拆分；归档软删除 | 真实 handler 已覆盖 CRUD、identity picking、link/unlink、merge/split、import 引用校验、派生历史统计；SidePanel smoke 覆盖 Client history UI | v0.7 最小闭环已验证；exact 自动 link 待实现 |
| FieldSelector / Selector Assist | `selectors:list/create/update/archive/startPicking/extractForCurrentPage` | 创建 selector；读取 active/all；更新 selector 并递增 version；归档软删除；capture/extract 更新 success/failure；profile extraction 可使用 selector source | 真实 handler 已覆盖 CRUD、startPicking、capture success/failure、extract failure、profile selector source、import 校验；SidePanel smoke 覆盖 Selector Assist UI | v0.9 最小闭环已验证；只允许用户点击后临时注入 |
| Analytics | `analytics:getSummary/getByScoreBand/getBySkill/getByClientType/getByTemplate` | 只读实时派生；不写事实，不写 cache；按 window/version/skill/scoreBand/clientType/template 过滤 | 真实 handler 已覆盖 metrics、groups、filters、low sample；Analytics 页面 smoke 覆盖主要 selector | v0.8 最小闭环已验证 |
...
| 风险边界 | `scripts/validate_v0_9.mjs` 静态扫描 | 检查自动化高风险 API 和 Upwork 私有 API 文本 | 有静态测试 | 只能证明代码文本没有命中，不能替代扩展运行时 smoke |

#### 9.9.2 已确认测试是否调用真实业务逻辑

当前 `scripts/validate_v0_9.mjs` 已真实调用：

1. `src/shared/adapters.js` 的 `mapRawProfileFields()`。
2. `src/shared/adapters.js` 的 `normalizeRawScore()`。
3. background 注册到 `chrome.runtime.onMessage` 的真实 handler，目前覆盖 migration、settings、capture、archive/restore/delete、notes、score、proposal、outcome、client、selector、analytics、backup/export/import。
...
MCP / Playwright 当前没有覆盖真实 background 业务逻辑，因为测试注入的是 mocked `chrome.runtime.sendMessage`。因此它只能证明 Options UI 按钮和状态文案流程正确；真实业务逻辑由 `scripts/validate_v0_9.mjs` 的 handler harness 覆盖。

#### 9.9.3 已纳入真实业务逻辑测试的 core case

这些测试通过真实 background message handler 调用业务入口。当前集中在 `scripts/validate_v0_9.mjs` 中 mock `chrome.storage.local`、`chrome.tabs.query`、`chrome.scripting.executeScript`、`fetch`，然后发送真实 message。
...
1. 测试覆盖不足。
   - 状态：`[已大幅修复]`。
   - 证据：`scripts/validate_v0_9.mjs` 已建立 background harness，以真实 message handler 覆盖 9.9.3 的 core case。
...
11. 未实现业务域不应被误判为完成。
    - 状态：`[已修复当前边界]`。
    - 证据：Selector Assist 已在 v0.9 建立完整最小业务闭环；ClientRecord 当前只允许人工创建/关联/合并/拆分，避免 probable match 自动污染。Analytics 已在 v0.8 实现只读实时派生闭环。
...
13. 未来实体 import validation 不完整。
    - 状态：`[已修复当前边界]`。
    - 决策：已实现的 My Profile / Portfolio / ProposalDraft / OutcomeEvent / ClientRecord / FieldSelector 允许导入并校验 shape/ref/sourceRefs/payload；仍未实现的 future cache 暂时拒绝非空导入；到对应版本实现时再补完整 validator 和引用校验。
    - 证据：`validateUnimplementedImportKeysAreEmpty()` 仍拒绝非空 `uosc_analytics_cache`；FieldSelector import 已校验 fieldKey、host、pageType、selector、version；ProposalDraft import 已校验 `inputScoreResultId` 和 `sourceRefs` 坏引用；OutcomeEvent import 已校验 proposal draft 和 detectedStatus 坏引用；ClientRecord import 已校验 `Opportunity.clientRecordId`、`identitySources` 和 active `primaryClientKey` 唯一性。

14. 计划版本和扩展发布版本关系不明确。
    - 状态：`[已修复]`。
    - 决策：计划版本绑定 Chrome extension `manifest.version`。
    - 证据：`src/shared/schema.js` 导出 `PLAN_VERSION = "0.9.0"`，`manifest.json` 使用 `version = "0.9.0"`，校验脚本断言两者一致。
...
1. P6：`[已完成]` v0.9 Selector Assist 已补 selector picking 的用户触发边界、overlay 清理规则、selector 版本/失效 contract 和真实扩展测试。
...
7. `[已实现]` import validation 已覆盖 core entity 未知字段、缺失 required field、引用不存在 id；FieldSelector 已纳入 validator；仍未实现的 future cache 非空导入已拒绝。
